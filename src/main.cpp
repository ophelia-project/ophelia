...
#if defined(NDEBUG)
# error "Ophelia cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Ophelia Signed Message:\n";
...
{
    // Ophelia: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 1 * 60; // Ophelia: 1 minute - 1440 blocks a day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ophelia: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ophelia: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2097bc0c1839a997bd49e6dfbe2a153b82837a0e50a78a7595fcf1b36439c1a7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x18a386628c2aa3b5fa152e3cb0d1d6ad217e3a051d2faa725bfe7bdcf0714b98"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Ophelia: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OpheliaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OpheliaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OpheliaMiner(CWallet *pwallet)
{
    printf("OpheliaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ophelia-miner");
    ...
    try { loop {
        ...
        printf("Running OpheliaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OpheliaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OpheliaMiner, pwallet));
}
